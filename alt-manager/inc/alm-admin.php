...
function alm_settings_admin() {
    ...
    // Fallback: explicit single-site save handler for AI settings when options.php flow fails (prevents "link expired")
    if ( !is_network_admin() && isset( $_SERVER['REQUEST_METHOD'] ) && 'POST' === $_SERVER['REQUEST_METHOD'] ) {
        // Plugin-specific AI settings save (when form posts to options-general.php?page=alt-manager)
        if ( isset( $_POST['alm_ai_settings_nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['alm_ai_settings_nonce'] ) ), 'alm_ai_settings_save' ) ) {
            if ( isset( $_POST['alm_use_ai_connectors'] ) || isset( $_POST['alm_ai_connector'] ) || isset( $_POST['alm_ai_api_key'] ) ) {
                alm_update_option( 'alm_use_ai_connectors', ( isset( $_POST['alm_use_ai_connectors'] ) ? 'enabled' : '' ) );
                $connector_val = ( isset( $_POST['alm_ai_connector'] ) ? sanitize_text_field( wp_unslash( $_POST['alm_ai_connector'] ) ) : (( isset( $_POST['alm_use_ai_connectors'] ) ? '' : 'plugin' )) );
                alm_update_option( 'alm_ai_connector', $connector_val );
                if ( isset( $_POST['alm_ai_api_key'] ) ) {
                    alm_update_option( 'alm_ai_api_key', sanitize_text_field( wp_unslash( $_POST['alm_ai_api_key'] ) ) );
                }
                wp_safe_redirect( esc_url_raw( admin_url( 'options-general.php?page=alt-manager&tab=ai_settings&settings-updated=1' ) ) );
                exit;
            }
        } elseif ( isset( $_POST['_wpnonce'] ) && !empty( $_POST['_wpnonce'] ) ) {
            // Fallback: Validate WP options nonce generated by settings_fields()
            if ( check_admin_referer( 'update-options' ) ) {
                // Save AI connector options if present
                if ( isset( $_POST['alm_use_ai_connectors'] ) || isset( $_POST['alm_ai_connector'] ) || isset( $_POST['alm_ai_api_key'] ) ) {
                    alm_update_option( 'alm_use_ai_connectors', ( isset( $_POST['alm_use_ai_connectors'] ) ? 'enabled' : '' ) );
                    $connector_val = ( isset( $_POST['alm_ai_connector'] ) ? sanitize_text_field( wp_unslash( $_POST['alm_ai_connector'] ) ) : (( isset( $_POST['alm_use_ai_connectors'] ) ? '' : 'plugin' )) );
                    alm_update_option( 'alm_ai_connector', $connector_val );
                ...
                // Nonce failed — will fall through to normal settings page messages
                error_log( '[alm ERROR] check_admin_referer failed for update-options' );
            }
        }
    }
    ...
    if ( '' === $active_tab ) {
        ...
        echo ( is_network_admin() ? '' : esc_url( admin_url( 'options.php' ) ) );
        ?>">
            <?php 
        if ( is_network_admin() ) {
        ...
        <form method="post" action="<?php 
        echo ( is_network_admin() ? '' : esc_url( admin_url( 'options-general.php?page=alt-manager' ) ) );
        ?>">
            <?php 
